...
| Vanilla URL | `http://localhost:5500/pages/signup.html` |
...
| 표기 | 의미 |
|---|---|
| `STATIC` | JSX·검증 함수·상태 분기를 코드로 확인 |
| `BACKEND-CODE` | Signup DTO·Service·Controller 계약 확인 |
| `BACKEND-TEST` | `UserServiceTest` 21건 재실행 |
| `LIVE-API` | localhost 백엔드에 실제 201·400·409 요청 |
| `E2E-REACT` | 이미지 선택과 폼 제출을 React 화면에서 실행 |
| `E2E-VANILLA` | Vanilla 화면에서 같은 시나리오 실행 |
| `MOCK` | 정상 endpoint로 만들 수 없는 지연·임의 409·500·네트워크 오류 |

## 화면과 이동

| ID | 테스트 | 기대 결과 | Vanilla 결과 | React 결과 | 비고 |
|---|---|---|---|---|---|
| SIGNUP-001 | 회원가입 페이지에 진입한다. | 모든 필드, 프로필 영역, 버튼과 이동 링크가 표시된다. | PASS | PASS | E2E-VANILLA·E2E-REACT |
| SIGNUP-002 | 최초 버튼 상태를 확인한다. | 회원가입 버튼이 비활성화되어 있다. | PASS | PASS | E2E-VANILLA·E2E-REACT |
...
| SIGNUP-011 | 이미지 파일을 선택한다. | 원형 영역에 미리보기가 표시되고 helper가 사라진다. | PASS | PASS | E2E-VANILLA·E2E-REACT |
...
| SIGNUP-035 | 모든 필수 값을 유효하게 만든다. | 버튼이 활성화되고 `active` 클래스가 적용된다. | PASS | PASS | E2E-VANILLA·E2E-REACT |
| SIGNUP-036 | 버튼을 빠르게 여러 번 선택한다. | API 요청이 한 번만 처리된다. | 미실행 | PASS | `isSubmitting` 조기 반환과 요청 중 disabled 처리 확인 |
| SIGNUP-037 | 요청을 지연시킨다. | 요청 중 버튼이 비활성화된다. | 미실행 | BLOCKED (MOCK) | 지연 응답 장치 필요, STATIC 분기는 확인 |

## API 요청과 성공

| ID | 테스트 | 기대 결과 | Vanilla 결과 | React 결과 | 비고 |
|---|---|---|---|---|---|
| SIGNUP-040 | 유효한 폼을 제출한다. | `POST /users/signup` 요청이 전송된다. | PASS | PASS | E2E-VANILLA·E2E-REACT·LIVE-API |
...
| SIGNUP-044 | 회원가입에 성공한다. | 완료 alert가 표시된다. | PASS | PASS | E2E-VANILLA·E2E-REACT |
| SIGNUP-045 | 완료 alert를 닫는다. | 로그인 페이지로 이동한다. | PASS | PASS | 두 화면 모두 alert 종료 후 로그인 이동 확인 |

## API 오류

| ID | 테스트 | 기대 결과 | Vanilla 결과 | React 결과 | 비고 |
|---|---|---|---|---|---|
| SIGNUP-050 | 400 `invalid_request`를 반환한다. | 입력값 확인 alert가 표시된다. | 미실행 | PASS | LIVE-API·STATIC |
| SIGNUP-051 | 400 `invalid_email_format`을 반환한다. | 이메일 형식 helper가 표시된다. | 미실행 | PASS | LIVE-API·STATIC |
| SIGNUP-052 | 400 `invalid_password_format`을 반환한다. | 비밀번호 형식 helper가 표시된다. | 미실행 | PASS | LIVE-API·STATIC |
| SIGNUP-053 | 400 `invalid_nickname_format`을 반환한다. | 닉네임 형식 helper가 표시된다. | 미실행 | PASS | LIVE-API·STATIC |
| SIGNUP-054 | 409 `email_already_exist`를 반환한다. | 이메일 중복 helper가 표시된다. | 미실행 | PASS | LIVE-API·E2E-REACT |
| SIGNUP-055 | 409 `nickname_already_exist`를 반환한다. | 닉네임 중복 helper가 표시된다. | 미실행 | PASS | LIVE-API·STATIC |
| SIGNUP-056 | 알 수 없는 409 message를 반환한다. | 사용 중인 정보 alert가 표시된다. | 미실행 | BLOCKED (MOCK) | 백엔드가 현재 두 종류의 409만 반환 |
| SIGNUP-057 | 서버가 500을 반환한다. | 서버 오류 alert가 표시된다. | 미실행 | BLOCKED (MOCK) | 정상 endpoint에 강제 500 장치 없음 |
| SIGNUP-058 | 네트워크 오류가 발생한다. | 일반 회원가입 오류 alert가 표시된다. | 미실행 | BLOCKED (MOCK) | 요청 차단 모의 필요 |
| SIGNUP-059 | 실패 응답 후 확인한다. | 입력값과 선택 이미지가 유지되고 버튼이 복구된다. | 미실행 | PASS | E2E-REACT, 이메일 중복 실패로 확인 |
...
- [x] 서버 중복 오류가 `UI-SIGNUP-10`, `UI-SIGNUP-11`과 일치한다. (LIVE-API·E2E-REACT)
- [x] 성공 결과가 `UI-SIGNUP-12`와 일치한다. (E2E-REACT)

## 테스트 중 발견 사항

| ID | 구분 | 내용 | 후속 작업 |
|---|---|---|---|
| SIGNUP-037 | MOCK 필요 | 서버 응답 지연 상태를 브라우저에서 재현하지 않음 | fetch 지연 또는 Mock Service Worker 도입 후 재검증 |
| SIGNUP-056~058 | MOCK 필요 | 정상 백엔드가 반환하지 않는 임의 409·강제 500·네트워크 단절 | LIVE-API와 구분되는 프론트 모의 응답 테스트로 검증 |
